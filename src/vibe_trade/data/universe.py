...
# S&P 500 components (as of early 2025 — update periodically)
SP500_SYMBOLS = [
...
def load_universe(config: UniverseConfig) -> list[str]:
    ...
    Deduplicates while preserving order — the scanner iterates this list
    once per scan, so duplicates would produce multiple orders for the
    same ticker, violating 'one order per ticker per day'.
    """
    if config.source == "custom":
        if not config.custom_symbols:
            logger.warning("Custom universe selected but no symbols provided")
            return []
        return list(dict.fromkeys(config.custom_symbols))
